...
import writingDataJson from "@/data/writing/mock-3.json";
...
export interface WritingTask {
    ...
    minWords: number;
    instructions: string[];
    image?: string; // absolute path from /public, e.g. "/photo/map3.png"
}
...
function isLegacySchema(v: unknown): v is LegacySchema {
    return isRecord(v) && ("task1" in v || "task2" in v);
}
        ...
        if (isArraySchema(raw)) {
            ...
            const tasks = raw.tasks.map((t, i): WritingTask => {
                ...
                return {
                    ...
                    title: String(
                        (obj.title as string) ?? (i === 0 ? "Writing Task 1" : "Writing Task 2"),
                    ),
            ...
            const tasks: WritingTask[] = [
                {
                    ...
                    instructions: Array.isArray(t1.instructions)
                        ...
                        : [],
                    image:
                        typeof t1.image === "string" ? (t1.image as string) : DEFAULT_T1_IMAGE,
                },
                {
                    id: "task2",
                    ...
                    instructions: Array.isArray(t2.instructions)
                        ...
                        : [],
                    image: typeof t2.image === "string" ? (t2.image as string) : undefined,
                },
            ];
            return { durationMinutes: dur, tasks };
        }
...
async function tgSend(text: string, parseMode?: "Markdown") {
    ...
    if (!res.ok) {
        console.error("Telegram error", res.status, await res.text().catch(() => "<no-body>"));
    }
}

/** Split long messages to respect 4096 char limit */
...
async function sendWritingToTelegram(payload: {
    ...
    const { firstName, lastName, phone, task1Words, task2Words, task1Text, task2Text } = payload;
...
function MobileTaskDrawer({
    ...
    return (
        <div className="fixed inset-0 z-50 bg-black/60 md:hidden" onClick={onClose}>
            ...
            >
                <button className="absolute top-2 right-4 text-2xl leading-none" onClick={onClose}>
                    ×
                </button>
...
/* ─────────────────── Main Component ─────────────────── */
export default function WritingMock3() {
    ...
    const LOCAL_KEY = "writing-mock3-answers";
    const LOCAL_USER_KEY = "writing-mock3-user";
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="mt-1 text-sm">
                                            Words: {wc} / {t.minWords} {!ok && <span className="text-red-600">(min!)</span>}
